			...
			if (receivedBitLen == BITLEN)        //32位数据接收完毕
			{
				...
				if (IrValue[2] = ~IrValue[3])   //数据码与其反码不符，有误
			    {
					switch (IrValue[2])
		            {
			            ...
			            case 0x5A:
						case 0x16: {g_startPosition = 448; g_mode = 1; g_characterLen = 64;}; break;                   //模式1静态显示
						
						case 0x42: {g_startPosition = 0; g_mode = 2; g_characterLen = 128; IrValue[2] = 0;}; break;    //模式2动态流动显示
						
						case 0x52: {g_startPosition = 512; g_mode = 3; g_characterLen = 128; IrValue[2] = 0;}; break;  //模式3动态流动显示
						
						case 0x4a: {g_startPosition = 2224; g_mode = 4; g_characterLen = 128; IrValue[2] = 0;}; break;  //模式4动态流动显示
		            }
			    }
			}
		}
